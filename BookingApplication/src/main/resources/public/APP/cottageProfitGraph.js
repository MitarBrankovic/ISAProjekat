Vue.component("CottageProfitGraph", {
    data: function() {
        return {
            activeUser:"",
			chartInfo:[],
	...
	weekFormat() {
	    ...
	    axios
        	.get('/cottageAppointments/getCottageProfitChartsWeek/' + instructorsId)
	        .then(response => {
	...
	monthFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageProfitChartsMonth/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
	...
	yearFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageProfitChartsYear/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
	...
	allFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageProfitCharts/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
