Vue.component("CurrentCottageAppointments", {
    data: function() {
        return {
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <td>{{appointment.cottage.name}}</td>
            <td>{{appointment.cottage.address}}</td>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
            	<div class="row my-row" style="margin-top: 2%;">
            		<span class="input-group-text" id="basic-addon1">Vikendica</span>
            		<select class="form-select" v-model="newAppointment.cottageId" aria-label="Example select with button addon">
			                  <option v-for="cottage in cottages" :value="cottage.id">{{cottage.name}} - {{cottage.address}}</option>
			                </select>
            	</div>
                <div class="row my-row" style="margin-top: 2%;">
    ...
    methods: {
    	...
    	async checkOverlap() {
        	var bool = false;
        	await axios
               .post('/cottageAppointments/checkOverlap', this.newAppointment)
               .then((response)=>{
               		bool = response.data
               		console.log(bool)
        ...
        addAppointment() {
        	axios
               .post('/cottageAppointments/addOwnersAppointmentForClient', this.newAppointment,{
                headers: {
        ...
        checkNewAppointment(){
        	if (this.newAppointment.cottageId != 0  && this.newAppointment.dateFrom !== "" && this.newAppointment.timeFrom !== "" &&  this.newAppointment.dateUntil !== "" && 
    ...
    mounted() {
        ...
        if(this.activeUser.role != 'cottage_owner')
         ...
         axios
            .get("cottageAppointments/getCurrentAppointments/" + this.activeUser.id)
        ...
        axios
        	.get('/cottages/getAllCottagesByOwner/' + this.activeUser.id)
	        .then(response => (
