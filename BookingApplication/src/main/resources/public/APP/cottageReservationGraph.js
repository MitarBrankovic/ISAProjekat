Vue.component("CottageReservationGraph", {
    data: function() {
        return {
            activeUser:"",
			chartInfo:[],
	...
	weekFormat() {
	    ...
	    axios
        	.get('/cottageAppointments/getCottageReservationChartsWeek/' + instructorsId)
	        .then(response => {
	...
	monthFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageReservationChartsMonth/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
	...
	yearFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageReservationChartsYear/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
	...
	allFormat() {
		instructorsId = this.activeUser.id 
        axios
        	.get('/cottageAppointments/getCottageReservationCharts/' + instructorsId)
	        .then(response => {
	        this.chartInfo = response.data
	        console.log(this.chartInfo)
			this.renderChart(this.makeSeries(this.chartInfo));
