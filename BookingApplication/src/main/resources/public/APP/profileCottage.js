	...
	<div class="col col-sm-6">
                        <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
						  ...
						  <div class="carousel-item active">
						      <img :src="cottage.photo" class="entityPhoto" alt="...">
						    </div>
...
<div class="row my-row">
                      	...
                      	<div class="col col-sm-4">
	                      	<div class="row my-row">
		                      	<div class="col col-sm-6">
				                	...
				                	<input id="uploadImage" v-if="activeUser != null && activeUser.role == 'cottage_owner'"  name="myPhoto" required @change=imageAddedNew type="file" accept="image/png, image/jpeg" class="form-control">
		                      	</div>
		                      	<div class="col col-sm-3">
		                      		<button type="button" v-if="activeUser != null && activeUser.role == 'cottage_owner' " style="margin-top: 15%" v-on:click="addPhoto()" class="btn btn-success">Dodaj sliku</button>
		                      	</div>
		                      	<div class="col col-sm-3">
		                      		<button type="button" v-if="activeUser != null && activeUser.role == 'cottage_owner' " style="margin-top: 15%" v-on:click="removePhoto()" class="btn btn-danger">Obriši sliku</button>
		                      	</div>
	                      	</div>
                      	</div>   
                      	</div>    
    ...
    <button type="button" style="margin-top: 3%; margin-bottom: 3%;" v-if="activeUser != null && activeUser.role == 'cottage_owner'" data-bs-toggle="modal" data-bs-target="#newAppointment" class="btn btn-danger btn-lg">Dodaj brzu rezervaciju</button>
	</div>
	<div>
	<div>
...
async checkOverlap() {
        	var bool = false;
        	await axios
               .post('/cottageAppointments/checkOverlap', this.quickAppointment)
               .then((response)=>{
               		bool = response.data
               		console.log(bool)
...
async checkCottageAvailability() {
        	var bool = false;
        	await axios
               .post('/cottageAppointments/checkCottageAvailability', this.quickAppointment)
               .then((response)=>{
               		bool = response.data
               		console.log(bool)
               		return response.data
...
async addNewAppointment(){
            this.quickAppointment.cottageId = this.cottage.id
            if (this.checkNewAppointment()){
            var overlap = await this.checkOverlap()
    		var instrAvail = await this.checkCottageAvailability()
            	if (this.checkDate()){
            		if (overlap) {
	            		if (1==1) {
			         	axios
			               .post('/cottageAppointments/addQuickAppointment', this.quickAppointment, {
                headers: {
                  'Authorization': `Bearer ${localStorage.jwt.slice(1,-1)}`
                },})
			               .then(response=>{
...
addPhoto(){
        this.newPhoto.entityId = this.cottage.id;
        if (this.newPhoto.photo != null){
         	axios
               .post('/cottages/addPhoto', this.newPhoto, {
                headers: {
                  'Authorization': `Bearer ${localStorage.jwt.slice(1,-1)}`
                },})
               .then(response=>{
        ...
        removePhoto(){
         	axios
               .post('/cottages/removePhoto/' + this.cottage.id, {
                headers: {
                  'Authorization': `Bearer ${localStorage.jwt.slice(1,-1)}`
                },})
               .then(response=>{
...
checkNewPricelistItem(){
   ...
   init: function(){
            const map = new ol.Map({
                ...
                view: new ol.View({
                  center: ol.proj.fromLonLat([this.cottage.longitude, this.cottage.latitude]),
                  maxZoom: 18,
                  zoom: 12
                })
              })
              ...
              var layer = new ol.layer.Vector({
                source: new ol.source.Vector({
                    features: [
                        new ol.Feature({
                            geometry: new ol.geom.Point(ol.proj.fromLonLat([this.cottage.longitude, this.cottage.latitude]))
                        })
                    ]
                })
            });
            ...
            map.on('singleclick', function (event) {
              if (map.hasFeatureAtPixel(event.pixel) === true) {
                  ...
                  content.innerHTML =  this.cottage.name;
          ...
          content.innerHTML = this.cottage.name;
          overlay.setPosition(ol.proj.fromLonLat([this.cottage.longitude, this.cottage.latitude]));
